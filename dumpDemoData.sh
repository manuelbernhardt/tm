...
mysqldump -u root --lock-all-tables tm $GENERAL_OPTS tm_ProjectRole_unitRoles --where="ProjectRole_id in (select id from tm_ProjectRole where account_id = $ACCOUNT_ID)" > projectrole_unitRoles.sql
mysqldump -u root --lock-all-tables tm $GENERAL_OPTS tm_ProjectWidget_parameters --where="ProjectWidget_id in (select id from tm_ProjectWidget where account_id = $ACCOUNT_ID)" > projectwidget_parameters.sql
...
mysqldump -u root --lock-all-tables tm $GENERAL_OPTS tm_User_accountRoles --where="TMUser_id in (select id from tm_User where account_id = $ACCOUNT_ID)" > tmuser_accountRoles.sql
