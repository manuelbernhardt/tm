...
mysqldump -u root --lock-all-tables tm $GENERAL_OPTS tm_User_tm_ProjectRole --where="TMUser_id in (select id from TMUser where account_id = $ACCOUNT_ID)" > tmuser_projectRole.sql
mysqldump -u root --lock-all-tables tm $GENERAL_OPTS tm_User_accountRoles --where="TMUser_id in (select id from TMUser where account_id = $ACCOUNT_ID)" > tmuser_accountRoles.sql
